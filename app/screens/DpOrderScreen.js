            ...
            return (
              <div>
                <div className="flex items-baseline justify-between mb-2">
                  <h3 className="font-bold text-sm text-pink-600">Items to Recreate This Look</h3>
                ...
                {Object.entries(grouped).map(([category, list]) => (
                  <div key={category} className="mb-3">
                    ...
                    {list.map((item, i) => (
                      <div key={item.id || `${category}-${i}`} className="flex items-center gap-2.5 py-2 border-b border-gray-50">
                        ...
                        {/* Item photo — decorators work by sight, not by SKU code or counts */}
                        {item.item_image_url ? (
                          <img
                            src={`${item.item_image_url}?tr=w-112,h-112,c-maintain_ratio`}
                            alt=""
                            className="w-12 h-12 rounded-xl object-cover border border-gray-200 bg-white shrink-0"
                          />
                        ) : (
                          <div className="w-12 h-12 rounded-xl bg-gray-50 border border-dashed border-gray-200 shrink-0" />
                        )}
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-gray-800">{item.name}</p>
                          {item.description && (
                            <p className="text-[10px] text-gray-500 italic leading-tight">{item.description}</p>
                          )}
                          {item.placement && (
                            <span className="inline-block mt-0.5 text-[9px] font-bold text-pink-600 bg-pink-50 border border-pink-100 rounded px-1.5 py-0.5">
                              📍 {item.placement}
                            </span>
                          )}
                        </div>
